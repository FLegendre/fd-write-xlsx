...
{
  int zip_error;
  auto const archive_ptr{ zip_open(xlsx_file_name, ZIP_CREATE | ZIP_TRUNCATE, &zip_error) };
  if (!archive_ptr)
...
<Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/>
    ...
    auto s{ zip_source_buffer(archive_ptr, str, std::strlen(str), 0) };
    if (s == nullptr) {
      zip_source_free(s);
      throw Exception{ "unable to get a buffer to add “[Content_Types].xml” file" };
    }
    auto r{ zip_file_add(archive_ptr, "[Content_Types].xml", s, 0) };
    if (r < 0) {
      zip_source_free(s);
  ...
  if (zip_dir_add(archive_ptr, "_rels", 0) < 0)
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/>
    ...
    auto r{ zip_file_add(archive_ptr, "_rels/.rels", s, 0) };
    if (r < 0) {
      zip_source_free(s);
  ...
  if (zip_dir_add(archive_ptr, "xl", 0) < 0)
...
<sheet name="sheet1" sheetId="1" r:id="rId1"/>
    ...
    auto r{ zip_file_add(archive_ptr, "xl/workbook.xml", s, 0) };
    if (r < 0) {
      zip_source_free(s);
  ...
  if (zip_dir_add(archive_ptr, "xl/_rels", 0) < 0)
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/>
    ...
    auto r{ zip_file_add(archive_ptr, "xl/_rels/workbook.xml.rels", s, 0) };
    if (r < 0) {
      zip_source_free(s);
  ...
  if (zip_dir_add(archive_ptr, "xl/worksheets", 0) < 0)
  ...
  // </worksheet>
  {
    ...
    zip_file_add(archive_ptr, "xl/worksheets/sheet1.xml", source_ptr, 0);
  }
  if (zip_close(archive_ptr) < 0)
